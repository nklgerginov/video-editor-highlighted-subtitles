...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QFileDArialog, QColorDialog, QApplication, QSizePolicy
)
...
class MainWindow(QMainWindow):
 ...
 dark_palette.setColor(QPalette.ColorRole.Disabled, QPalette.ColorRole.Text, QColor(127, 127, 127))
    ...
    self.stop_button top_button = QPushButton("Stop")
...
Arial", "Helvetica", "Roboto", "Segoe UI"]:
    ...
    def _choose_text_color(self):
        color = QColorDArialog.getColor(self.text_color, self, "Choose Text Color")
        if color.isValid():
    ...
    def _choose_highlight_color(self):
        color = QColorDArialog.getColor(self.highlight_color, self, "Choose Highlight Color")
        if color.isValid():
...
self.project.position = pos
    ...
    def _load_video(self):
        fp, _ = QFileDArialog.getOpenFileName(self, "Open Video", "", "Video Files (*.mp4 *.avi *.mov *.mkv)")
        if fp:
...
{error}")
    ...
    def _export(self):
        ...
        op, _ = QFileDArialog.getSaveFileName(
